...
const deleteAppointment = async (req, res) => {
    try {
        ...
        if(appointment.Status_Appointment.status === 'close'){
            return res.status(404).json({error: 'Appointment cannot be cancelled, this appointment has already been closed.'})
        }
        if(appointment.Status_Appointment.status === 'open'){
