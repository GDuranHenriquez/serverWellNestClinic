...
async function postAppointment(req, res){
  try {
    ...
    const [appointment, created] = await Appointment.findOrCreate({where: {date, doctor}, defaults:{startTime, endTime}});
    if(created) {
      appointment.setAppointment_UserClient(userClient)
        return res.status(200).json(appointment)
    } else {
        return res.status(403).json({error: 'At that date the doctor already have a scheduled appointment'})
    }
  } catch (error) {
